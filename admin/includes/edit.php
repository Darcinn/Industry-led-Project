...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['user_id'])) {
  ...
  if (!empty($_POST['contact_no'])) {
    ...
    $q = "UPDATE users SET contact_no='$co' WHERE user_id='$id'";
    $r = @mysqli_query($link, $q);
  }

  if (!empty($_POST['account_type'])) {
    $at = trim($_POST['account_type']);
    $q = "UPDATE users SET account_type='$at' WHERE user_id='$id'";
    $r = @mysqli_query($link, $q);
  }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['new_genre'])) {
  ...
  if (!empty($_POST['new_genre'])) {
    ...
    $g = trim($_POST['new_genre']);

    $q = "SELECT * FROM genre WHERE genre_name='$g'";
    $r = @mysqli_query($link, $q);

    if (mysqli_num_rows($r) == 0) {
      $q = "INSERT INTO genre (genre_name) VALUES ('$g')";
      $r = @mysqli_query($link, $q);
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['tv_id'])) {
  ...
  if (!empty($_POST['tv_genre'])) {
    ...
    $q = "UPDATE tv SET tv_genre='$tg' WHERE tv_id='$id'";
    $r = @mysqli_query($link, $q);
  }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['new_tv'])) {
  ...
  if (!empty($_POST['new_tv'])) {
    $q = "INSERT INTO tv (tv_title, tv_desc, tv_genre, tv_img, tv_trailer) VALUES ('$t', '$d', '$g', '$i', '$tr')";
    $r = @mysqli_query($link, $q);
  } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['episode_id'])) {
  ...
  if (!empty($_POST['episode_img'])) {
    ...
    $q = "UPDATE episode SET episode_img='$ei' WHERE episode_id='$id'";
    $r = @mysqli_query($link, $q);
  }
